      ...
      <table className="min-w-full divide-y divide-gray-200">
          ...
          <tr>
            {columns.map((column, index) => (
              <th
                key={index}
                scope="col"
                className="px-6 py-4 text-xs font-semibold tracking-wide text-left text-primary-800 uppercase bg-transparent"
              >
                {column.header}
              </th>
            ))}
            {actions && (
              <th scope="col" className="px-6 py-4 text-xs font-semibold tracking-wide text-center text-primary-800 uppercase bg-transparent">
                Aksi
              </th>
            )}
          </tr>
        ...
        <tbody className="bg-white divide-y divide-gray-200">
          {data.length > 0 ? (
                    ...
                    <div className="flex items-center justify-center space-x-2 min-h-[36px]">
                      {actions.map((action, actionIndex) => {
                        ...
                        return (
                          ...
                          >
                            {IconComponent ? <IconComponent className="inline w-4 h-4 mr-1" /> : null}
                            {action.label}
                          </button>
                        );
                      })}
            ...
            >
              <td colSpan={columns.length + (actions ? 1 : 0)} className="px-6 py-12 text-sm text-center text-gray-500 bg-gray-50">
                <div className="flex flex-col items-center justify-center">
                  <svg className="w-12 h-12 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9.172 16.172a4 4 0 015.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                  </svg>
                  <p className="mt-4 font-medium">Tidak ada data</p>
                  <p className="text-gray-500">Data akan muncul di sini ketika tersedia</p>
                </div>
              </td>
            </motion.tr>
          )}
